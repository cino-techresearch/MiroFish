...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, check for an already-completed report first
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # If the MD file does not exist, write a temporary file
            import tempfile
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Interview is unlocked only after the report is completed
...
def get_agent_log(report_id: str):
    ...
    Retrieves each step taken during report generation in real time, including:
...
def stream_agent_log(report_id: str):
    """
    Get the complete Agent log (all entries at once)
...
def get_console_log(report_id: str):
    ...
    Unlike the agent-log endpoint which returns structured JSON logs, this returns
    plain-text console-style log lines.
...
def stream_console_log(report_id: str):
    """
    Get the complete console log (all entries at once)
