...
# Safe simulation_id pattern (blocks path traversal)
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required file list (scripts excluded; scripts live in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - preparing: preparation complete if config_generated=True
        ...
        # Note: "failed" is NOT treated as prepared — a FAILED injection path that has
        # partial artifacts (config_generated=True) must not be misidentified as "already
        # prepared", which would prevent re-prepare. Use force_regenerate to retry.
        ...
        if status in prepared_statuses and config_generated:
            ...
            # TS-015 / FR-007: This function is read-only — it does not mutate state.json.
            # State transitions (preparing -> ready) are owned explicitly by prepare_simulation.
            # When preparing + config_generated + files exist, we only *report* "prepared" (no disk write).
            if status == "preparing":
                status = "ready"
...
def prepare_simulation():
    ...
    4. Intelligently generate the simulation configuration via LLM (with retry logic).
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // optional, whether to generate personas via LLM
    ...
    try:
        ...
        # ========== Synchronously fetch entity count (before spawning the background task) ==========
        # This lets the frontend know the expected total number of Agents immediately after calling prepare.
        try:
            logger.info(f"Synchronously fetching entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick entity read (edge info not needed, count only)
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist; if simulation_id is provided, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates over the reports directory, finds reports matching simulation_id,
...
def get_simulation_history():
    ...
    Used for displaying history on the home page; returns a simulation list enriched
    with project name, description, and other metadata.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read user-configured actual round count from run_state.json)
            ...
            # Get file list for the associated project (up to 3 files)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Fetch Agent Profiles for a simulation in real time (for live progress viewing during generation).

    Differences from the /profiles endpoint:
    - Reads files directly without going through SimulationManager.
    - Suited for real-time viewing during the generation process.
    - Returns additional metadata (e.g., file modification time, whether generation is ongoing).
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Fetch the simulation configuration in real time (for live progress viewing during generation).

    Differences from the /config endpoint:
    - Reads files directly without going through SimulationManager.
    - Suited for real-time viewing during the generation process.
    - Returns additional metadata (e.g., file modification time, whether generation is ongoing).
    - Can return partial information even when the configuration has not yet been fully generated.
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - agent_configs: Activity configuration per Agent (activity level, post frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download a simulation run script file (generic scripts located in backend/scripts/).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "force": false                         // optional: force restart (stops a running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, stops and clears run logs if the simulation is already running or completed.
        - Cleared content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Does not clear configuration files (simulation_config.json) or profile files.
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete; check whether a process is currently running
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for displaying real-time activity in the frontend.
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for displaying the progress bar and timeline view in the frontend.
...
def get_agent_stats(simulation_id: str):
    ...
    Used for displaying Agent activity rankings, action distributions, etc. in the frontend.
    """
    try:
...
def interview_agent():
    ...
    (waiting for commands after completing the simulation loop).
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if omitted: interviews both platforms simultaneously in dual-platform simulations
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // platform defaults if not specified
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if omitted: interviews both platforms for each Agent simultaneously
    ...
    try:
        ...
        # Optimize each interview item's prompt by adding prefix to prevent the Agent from calling tools
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What is your overall take on this?",  // required, question (same for all Agents)
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, get interview history for a specific Agent only
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation so it exits gracefully from
...
def upload_profiles():
    """Upload neutral JSON profiles and save them as injected_profiles.json (FR-004, NFR-004).

    Input (one of the following):
    - JSON body: {"simulation_id": "...", "profiles": [ {...}, ... ]}
    - multipart/form-data: simulation_id field + file(.json)

    Security: size exceeded (413, Flask MAX_CONTENT_LENGTH) / non-JSON extension (400) /
    schema violation (400) / path traversal (400).
    """
    try:
        # Collect simulation_id (form takes precedence, then json)
        ...
        # Block path traversal — only whitelist pattern allowed
        ...
        # Only allow uploads to existing simulations (prevents orphan directory / cross-sim input injection)
        manager = SimulationManager()
        ...
        # Save as injected_profiles.json
        sim_dir = manager._get_simulation_dir(simulation_id)
