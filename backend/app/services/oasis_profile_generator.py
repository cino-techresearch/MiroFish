...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        searched separately and results are merged. Parallel requests are used
        ...
        try:
            # Run edges and nodes search in parallel
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if zep_results.get("facts"):
            # Deduplicate: exclude already-known facts
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Return True if entity_type represents an individual."""
    ...
    def _generate_profile_with_llm(
        ...
        # Try multiple times until success or max attempts reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # max_tokens not set — let the LLM generate freely
                )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair JSON truncated by max_tokens output limit."""
        import re
        ...
        # Simple heuristic: if the last character is not a delimiter, the string may be cut off.
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Replace literal newlines inside string values
            ...
            try:
                ...
                # 5. More aggressive repair: strip control characters
                try:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation that faithfully reflect reality. You must return valid JSON; no unescaped newlines inside string values."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity below, faithfully reflecting reality as much as possible.
...
2. persona: Detailed persona description (~2000 words, plain text), covering:
   ...
   - Social media behavior (posting frequency, content preferences, interaction style, language)
   - Stance and opinions (attitude toward topics, content that provokes or moves them)
   - Distinctive traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (key part of the persona: individual's connection to the event, and actions/reactions already taken)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for the institution/group entity below, faithfully reflecting reality as much as possible.
...
2. persona: Detailed account profile description (~2000 words, plain text), covering:
   ...
   - Stance and attitude (official position on key topics, how controversies are handled)
   - Special notes (audience profile represented, operating habits)
   - Institutional memory (key part of the persona: institution's connection to the event, and actions/reactions already taken)
...
5. mbti: MBTI type describing the account style (e.g., ISTJ for rigorous and conservative)
6. country: Country name in English (e.g., "China")
...
- persona must be a single continuous text description without newline characters
...
- Institutional account speech must match its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        """
        Generate Agent Profiles from a list of entities in bulk (supports parallel generation).
        ...
        total = len(entities)
        profiles = [None] * total  # Pre-allocate to preserve order
        completed_count = [0]  # Use list so the closure can mutate it
        lock = Lock()
        ...
        def save_profiles_realtime():
            ...
            with lock:
                # Filter profiles that have been generated
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Print the generated persona to console in real time
        ...
        print(f"\n{'='*60}")
        print(f"Starting agent persona generation - {total} entities, {parallel_count} parallel workers")
        print(f"{'='*60}\n")
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Print to console only (logger no longer outputs full content to avoid duplication)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to file (choosing the correct format for the platform).
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                row = [
                    ...
                    _csv_safe(profile.name),          # name: real name
                    _csv_safe(profile.user_name),     # username: username
                    _csv_safe(user_char),             # user_char: full persona (internal LLM use)
                    _csv_safe(description)            # description: short bio (external display)
                ]
                writer.writerow(row)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # English mapping (LLM now generates English gender values)
        gender_map = {
            "male": "male",
            "female": "female",
            "other": "other",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # OASIS required fields - ensure defaults are present
