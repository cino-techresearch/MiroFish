...
class ReportLogger:
    ...
    Each line is a complete JSON object containing timestamp, action type, and detailed content.
    """
    ...
    def log_section_full_complete(
        ...
        The frontend should listen to this log to determine whether a section is truly
        finished and to retrieve the full content.
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    Writes console-style logs (INFO, WARNING, etc.) to a console_log.txt file in the
    report folder. These logs differ from agent_log.jsonl in that they are plain-text
    console output.
    """
...
This tool retrieves the complete overview of simulation results, especially suited for
understanding how events evolved. It will:
...
[Use Cases]
- When you need the complete development timeline of an event
- When you need to compare opinion changes across different stages
...
running simulation agents! This is not LLM simulation — it calls the actual interview
interface to obtain raw answers from simulation agents. By default, interviews are
conducted simultaneously on both Twitter and Reddit for a more comprehensive view.
...
- When you need to collect multiple opinions and positions
- When you need real answers from simulation agents (from the OASIS simulation environment)
- When you want the report to be more vivid by including "interview transcripts"
...
[Core Concept]
We have built a simulated world and injected a specific "simulation requirement" as a variable into it.
The evolution of the simulated world is a prediction of what may happen in the future.
What you are observing is not "experimental data" but a "rehearsal of the future."
...
[Report Positioning]
- This is a future prediction report based on simulation, revealing "if this, then what in the future"
...
- No sub-sections needed; each section should be written with complete content directly
...
The variable (simulation requirement) injected into the simulated world: {simulation_requirement}
...
1. Under the conditions we set, what state did the future arrive at?
...
You are an expert writer of "Future Prediction Reports" and are writing one section of the report.
...
(simulation requirements) into it; the behaviors and interactions of agents in the
simulation are predictions of future human behavior.
...
2. [Must cite agents' original statements and behaviors]
   - Agent statements and behaviors are predictions of future human behavior
   - Show these predictions in the report using citation format, for example:
     > "A certain group of people will say: original content..."
   - These citations are the core evidence of the simulation predictions

3. [Language consistency — cited content must be translated into the report language]
   - Tool-returned content may contain expressions in a language different from the report language
   - The report must be written entirely in the language specified by the user
   - When you cite tool-returned content in another language, you must translate it into the
     report language before writing it in
   - Keep the original meaning intact and ensure natural, fluent expression
...
4. [Faithfully present prediction results]
   ...
   - If certain information is insufficient, state so honestly

═══════════════════════════════════════════════════════════════
[Format Specifications — Critically Important!]
═══════════════════════════════════════════════════════════════

[One Section = Minimum Content Unit]
- Each section is the smallest block unit in the report
- It is FORBIDDEN to use any Markdown headings (#, ##, ###, #### etc.) within a section
- It is FORBIDDEN to add the section's main heading at the start of the content
- The section heading is added automatically by the system; you only write the body content
- Use **bold text**, paragraph breaks, quotes, and lists to organize content — not headings

[Correct Example]
```
This section analyzes the public opinion dissemination dynamics of the event. Through in-depth
analysis of the simulation data, we found...

**Initial Ignition Phase**

Weibo, as the first scene of public opinion, played a core role in initial information release:

> "Weibo contributed 68% of the initial voice share..."
...
- insight_forge: Deep insight analysis, automatically decomposes the problem and retrieves facts and relationships from multiple dimensions
...
The system will execute the tool and return the result to you. You do not need to and must not
write the tool return result yourself.

Option B - Output the final content:
Once you have gathered enough information through tools, output the section content starting
with "Final Answer:".
...
4. [Citation format requirements — must stand alone as a paragraph]
   Citations must be independent paragraphs with one blank line before and after;
   ...
   > "The school's response pattern appeared rigid and slow in the fast-changing
...
2. You must call tools first to retrieve simulation data before starting
...
- Start directly with body text; use **bold text** instead of sub-section headings

Please begin:
1. First think (Thought) about what information this section needs
2. Then call a tool (Action) to retrieve simulation data
3. After gathering enough information, output the Final Answer (body text only, no headings)"""
...
- If information is sufficient: start with "Final Answer:" and output the section content (must cite the above original text)
- If more information is needed: call one tool to continue retrieval
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called tools {tool_calls_count} time(s); at least {min_tool_calls} calls are required. "
    "Please call more tools to retrieve additional simulation data, then output the Final Answer. {unused_hint}"
)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately output the section content starting with "Final Answer:" based on the information gathered so far.'
)
...
2. Answer directly; avoid lengthy reasoning narratives
...
[Answer Style]
- Concise and direct; no lengthy elaborations
...
class ReportAgent:
    ...
    1. Planning phase: analyze simulation requirements, plan the report table of contents
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # In-depth interview — calls the real OASIS interview API for dual-platform agent responses
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when Format 1 does not match, to avoid false positives from JSON in body text
    ...
    def _get_tools_description(self) -> str:
        """Generate a tools description text."""
    ...
    def plan_outline(
        ...
        Uses LLM to analyze simulation requirements and plan the report table of contents.
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None; break and enter forced finalization
                break
            ...
            if has_final_answer:
                # Not enough tool calls — reject and require more
            ...
            if has_tool_calls:
                # Tool quota exhausted — notify and require Final Answer output
            ...
            if tool_calls_count < min_tool_calls:
                # Not enough tool calls — suggest unused tools
                unused_tools = all_tools - used_tools
                unused_hint = f"(These tools have not been used yet, recommended: {', '.join(unused_tools)})" if unused_tools else ""
            ...
            # Enough tool calls made; LLM output content without "Final Answer:" prefix.
            # Use this content directly as the final answer to avoid idle spinning.
    ...
    def generate_report(
        ...
        waiting for the entire report to finish.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        This is the console output log (INFO, WARNING, etc.) from the report generation
        process, different from the structured logs in agent_log.jsonl.

        Args:
            report_id: Report ID
            from_line: Line number to start reading from (0 means from the beginning;
                       used for incremental retrieval)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it duplicates the section title (within the first 5 lines)
                if i < 5:
                ...
                # because the section heading is added by the system; no heading should appear in content
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assembles the full report from saved section files and applies heading cleanup.
        """
        folder = cls._get_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep the report main heading (#) and section headings (##);
           convert all other levels (###, #### etc.) to bold text
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate heading (same content within the preceding 5 lines)
                is_duplicate = False
