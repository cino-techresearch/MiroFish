...
generating overly long content in a single pass:
...
class AgentActivityConfig:
    ...
    # Influence weight (determines the probability that posts are seen by other agents)
...
class TimeSimulationConfig:
    ...
    # Time represented per round (simulation minutes) - default 60 minutes (1 hour), accelerates time flow
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions required to trigger spread)
    viral_threshold: int = 10

    # Echo chamber effect strength (degree of clustering by similar opinions)
...
class SimulationConfigGenerator:
    ...
    information to automatically generate the optimal simulation parameter configuration.
    ...
    def generate_config(
        ...
        # If injection profiles are provided, agent count is based on profile count (independent of entity count, FR-005 redesign)
        ...
        # ========== Steps 3-N: Agent configuration ==========
        if agent_profiles is not None:
            # FR-005 redesign: when profiles are injected, derive agent_config directly from injection profiles.
            # Allows arbitrary count and arbitrary user_id regardless of ZEP entity count, with 0 LLM calls.
...
### Basic principles (reference only — adjust flexibly based on the specific event and participant group):
- Infer the timezone and daily schedule of the target user group from the simulation scenario; the following is a reference example for UTC+8 (China Standard Time)
- Almost no activity from midnight to 5am (activity multiplier 0.05)
- Gradually active from 6am to 8am (activity multiplier 0.4)
- Moderate activity during work hours 9am-6pm (activity multiplier 0.7)
- Peak hours in the evening 7pm-10pm (activity multiplier 1.5)
- Activity declines after 11pm (activity multiplier 0.5)
...
- **Important**: The example values below are for reference only; adjust specific time slots based on the nature of the event and characteristics of the participant group
  - Example: student peak may be 9pm-11pm; media is active all day; official institutions only during work hours
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format; time configuration must match the daily schedule of the target user group in the simulation scenario."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result and validate that agents_per_hour does not exceed total agent count"""
        # Get raw values
...
**Important**: poster_type must be chosen from the "available entity types" above so that initial posts can be assigned to appropriate agents for publishing.
For example: official announcements should be posted by Official/University type entities, news by MediaOutlet, student opinions by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        Matches the most appropriate agent_id based on the poster_type of each post
        """
        ...
        # Type alias table (handles different formats that LLM may output)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            # Attempt to find a matching agent
    ...
    def _agent_configs_from_profiles(self, profiles: List[Any]) -> List[AgentActivityConfig]:
        """Derive agent_config directly from injection profiles (FR-005 redesign, 0 LLM calls).

        agent_id uses profile user_id as-is, so arbitrary count and arbitrary user_id
        injection profiles are allowed regardless of ZEP entity count or 0-based continuity.
        Activity parameters use dataclass defaults.
        """
...
- **Time should match the target user group's daily schedule**: the following is a reference (UTC+8); adjust based on the simulation scenario
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, considering daily schedule>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON; configurations must match the daily schedule of the target user group in the simulation scenario."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single agent configuration by rule (Chinese daily schedule)"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Expert/professor: active during work hours and evenings, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
