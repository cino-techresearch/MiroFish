...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # If max_rounds is specified, append to command-line args
            if max_rounds is not None and max_rounds > 0:
            ...
            # Set working directory to the simulation directory (database files will be generated here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensures termination when server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'en'):
        ...
        try:
            ...
            # After the process ends, read the logs one final time
            if os.path.exists(twitter_actions_log):
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # Check which platforms are enabled (by whether the file exists)
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force terminate, /T = terminate tree (including child processes)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the record's platform field, otherwise use default
    ...
    def get_all_actions(
        ...
        """
        Get the complete action history across all platforms (no pagination limit)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up (avoid printing useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        # In non-debug mode this env var is absent, so cleanup should always be registered.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode, only register in the reloader child process;
        # in non-debug mode, always register
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms simultaneously and return a merged result
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation so it exits the command-waiting
        mode gracefully.
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specified Agent ID (optional; get history for that agent only)
