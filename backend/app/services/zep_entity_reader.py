...
class ZepEntityReader:
    ...
    2. Filter nodes that match predefined entity types (nodes whose Labels contain more than just "Entity").
    ...
    def _call_with_retry(
        ...
        Args:
            func: The function to execute (a no-arg lambda or callable).
            operation_name: Name of the operation, used in log messages.
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Fetch all nodes in the graph (with pagination).
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Fetch all edges in the graph (with pagination).
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Fetch all edges related to the specified node (with retry logic).
        ...
        """
        try:
            # Call the Zep API with retry logic
    ...
    def filter_defined_entities(
        ...
        - If a node's Labels include a label other than "Entity" or "Node", it matches a predefined type and is kept.

        Args:
            graph_id: Graph ID.
            defined_entity_types: Optional list of predefined entity types; if provided only these types are kept.
        ...
        # Fetch all edges for relationship lookup
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []

        # Build a UUID-to-node-data mapping
        ...
        for node in all_nodes:
            ...
            if enrich_with_edges:
                ...
                # Collect basic information for related nodes
                related_nodes = []
    ...
    def get_entity_with_context(
        ...
        """
        Fetch a single entity with its full context (edges and related nodes, with retry logic).
        ...
        try:
            ...
            # Collect related node information
            related_nodes = []
            for related_uuid in related_node_uuids:
                if related_uuid in node_map:
                    related_node = node_map[related_uuid]
