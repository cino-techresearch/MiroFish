...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        Uses natural language description format so Zep can extract entities and
        relationships. No simulation-related prefixes are added to avoid misleading
        graph updates.
        """
        ...
        # Return "agent_name: activity description" format directly, without a simulation prefix
    ...
    def _describe_like_post(self) -> str:
        """Like a post — includes post content and author information"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post — includes post content and author information"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_quote_post(self) -> str:
        """Quote post — includes original post content, author information, and the quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    to the Zep graph in real time. Activities are grouped by platform; a batch
    is sent to Zep once BATCH_SIZE activities have accumulated for a platform.

    All meaningful actions are updated to Zep. action_args contains full
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full contextual information (e.g., post content,
        usernames).
    ...
    def add_activity_from_dict(self, data: Dict[str, Any], platform: str):
        ...
        """
        # Skip entries that are event-type records
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Pause between sends to avoid issuing requests too quickly
    ...
    def _flush_remaining(self):
        ...
        # Then send whatever remains in each platform's buffer (even if below BATCH_SIZE)
        with self._buffer_lock:
