...
Wraps graph search, node reads, edge queries, etc. for use by Report Agent

Core retrieval tools (optimized):
1. InsightForge (deep insight retrieval) - most powerful hybrid retrieval, auto-generates sub-questions, multi-dimensional retrieval
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (cite these excerpts in the report)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (at sentence boundary, not hard truncation)
...
class ZepToolsService:
    ...
    def search_graph(
        ...
        Uses hybrid search (semantic + BM25) to search the graph for relevant information.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all graph edges (with pagination and temporal information)
    ...
    def insight_forge(
        ...
        entity_insights = []
        node_map = {}  # Used for relationship chain construction below
...
Return the sub-question list in JSON format."""
    ...
    def panorama_search(
        ...
        2. Gets all edges (including expired/invalid)
        3. Categorizes current active and historical information

        This tool is suitable for scenarios requiring full event context or tracking evolution.
        ...
        for edge in all_edges:
            ...
            if is_historical:
                ...
                valid_at_str = edge.valid_at or "Unknown"
                invalid_at_str = edge.invalid_at or edge.expired_at or "Unknown"
                fact_with_time = f"[{valid_at_str} - {invalid_at_str}] {edge.fact}"
    ...
    def interview_agents(
        ...
        5. Integrates all interview results into an interview report
        ...
        try:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may have stopped, please ensure OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable Agents to interview from the simulation Agent list based on the interview requirements.
...
1. Distill the main viewpoints of all parties
