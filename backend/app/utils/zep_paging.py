...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page, retrying with exponential backoff on transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch all graph nodes with pagination, returning at most max_items (default 2000). Each page request includes its own retry logic."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, returning the complete list. Each page request includes its own retry logic."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
