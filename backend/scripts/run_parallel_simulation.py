...
# This fixes issues where OASIS third-party libraries read files
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard streams to UTF-8 (fixes garbled console output)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force-set the default encoding (affects the default encoding of open())
    # Note: this needs to be set at Python startup; setting at runtime may have no effect.
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wraps open() to default to UTF-8 encoding for text mode.
        This fixes issues where third-party libraries (e.g., OASIS) read files
        without specifying an encoding.
        """
...
# The script is always located in backend/scripts/
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai warnings about max_tokens (we intentionally do not set max_tokens, letting the model decide)"""
...
def disable_oasis_logging():
    ...
    OASIS logs are too noisy (recording every agent observation and action);
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing the result, or a dict containing an error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None / not specified: interview on both platforms, returns merged result
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to display real entity names instead of
    placeholder labels like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid read in the previous call (rowid is used instead of
                    created_at because different platforms use different created_at formats)
        agent_names: Mapping of agent_id -> agent_name

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions; each element contains agent_id, agent_name,
                        action_type, and action_args (with context information)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Slim down action_args to key fields only (keep full content, no truncation)
            simplified_args = {}
            ...
            # Map action type name to canonical form
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: add original post content, author, and quote comment
            ...
            # Fetch the quote comment content (quote_content) of the quoting post
            if new_post_id:
                cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    When a boost LLM is configured, different platforms in the parallel simulation
    can use different API providers to increase concurrency.
    ...
    # Fall back to config if the model name is not in .env
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Build real agent name mapping from the config file (using entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # For any agent not in the config, fall back to the OASIS default name
    ...
    result.env = oasis.make(
        ...
        semaphore=30,  # Cap concurrent LLM requests to prevent API overload
    )
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed rowid in the database (rowid avoids created_at format discrepancies)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end with actions_count=0 when there are no active agents
            if action_logger:
    ...
    # Note: environment is not closed here; it is kept alive for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (falls back to general config if unavailable)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation completes, skip command-wait mode'
    )
    ...
    # Initialize logging (disable OASIS logs, clean up old files)
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actual rounds to execute: {args.max_rounds} (capped)")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: the process does not exit after the simulation
    completes; it waits for interview commands instead.
