...
This script reads parameters from a config file to execute the simulation fully automated.
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally omit max_tokens to let the model decide)"""
...
# Add the filter at module load time so it takes effect before camel code runs
...
class TwitterSimulationRunner:
    ...
    # Available Twitter actions (INTERVIEW excluded; it can only be triggered via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        ...
        Reads configuration from the project root .env file (highest priority):
        - LLM_API_KEY: API key
    ...
    def _get_active_agents_for_round(
        ...
        """
        Determine which agents to activate for the current round based on time and config.
        ...
        # Calculate activation probability per agent based on config
        candidates = []
    ...
    async def run(self, max_rounds: int = None):
        ...
        # Optionally enter command-waiting mode
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional, used to cap very long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
...
def setup_signal_handlers():
    """
    Set up signal handlers to ensure the process exits cleanly on SIGTERM/SIGINT.
    Gives the program a chance to release resources (close database, environment, etc.) gracefully.
    """
