...
// Toggle expand/collapse state for a self-loop item
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Check whether the edge direction matches the canonical direction (source UUID < target UUID)
    ...
    if (totalCount > 1) {
      ...
      // Curvature range grows with edge count — more edges, wider spread
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // Flip curvature when edge direction is reversed relative to canonical direction
      // so all edges are distributed in the same reference frame and do not overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust distance dynamically based on the number of edges between the pair
    ...
    // Add centering forces so isolated node clusters gather toward the center
  ...
  // Links — use path elements to support curved edges
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // Draw arc (sweep-flag=1 clockwise)
    ...
    // Compute quadratic bezier control point — adjusted by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset proportional to distance — ensures curves remain visible
  ...
  // Compute the midpoint of a link curve (used for label placement)
  ...
  // Link label backgrounds (white background improves text legibility)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Let simulation cool down only if a real drag occurred
        if (d._isDragging) {
