<template>
  ...
  >
    <!-- Background decoration: technical grid lines (shown only when projects exist) -->
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show overflow hint when there are more files -->
...
const containerStyle = computed(() => {
  ...
  // Compute required height: rows * card height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute card count in the current row to center each row
    ...
    const x = offset * 35
    // Adjust starting position: close to title while keeping appropriate spacing
...
// Get progress style class based on round progress
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record the latest target regardless of animation)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to execute (may have been overridden by later scroll)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Add a short delay before executing to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// Watch for route changes and reload data when returning to home
...
// Reload data when component is activated (keep-alive)
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Positioned from top-left; height changes only expand downward without shifting existing grid */
...
.cards-container {
  ...
  /* min-height is dynamically computed by JS, adapts to card count */
}
