...
// Get the username corresponding to an agent_id
...
// Compute total related topics count across all personas
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Emit message log (avoid duplicates)
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit config generation stage log (avoid duplicates)
...
onMounted(() => {
  // Automatically start prepare flow
  if (props.simulationId) {
