...
// Track the previous round per platform to detect changes and emit logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation has completed (via runner_status or per-platform completion flags)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: platform may report completion before backend updates runner_status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine enabled platforms by actions_count or running flag
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted

  // Return false if no platform is enabled
