            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but section may not be fully done) -->
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (excluding final summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "Student", "Education Worker", etc.)
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if first question is prefixed with "1."
      ...
      // Extract key quotes (supports multiple quote formats)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (English format, new backend format)
      // 2. "1. " or "\n1. " (number + dot, backward compat)
      let matches = []
      let match
...
const renderMarkdown = (content) => {
  ...
  // Remove leading second-level headings (## xxx), as section titles are shown in the outer layer
  ...
  // Clean <p><br> immediately before block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tag immediately following block-level element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: when single <ol> items are separated by paragraph content, keep incrementing
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Chinese "final answer:" prefix
  const chineseFinalMatch = response.match(/Final\s*Answer[:：]\s*\n*([\s\S]*)$/i)
  ...
  // If content is long and contains markdown, try removing thought process first
