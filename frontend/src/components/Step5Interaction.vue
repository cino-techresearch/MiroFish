...
const renderMarkdown = (content) => {
  ...
  // Remove <br> tags immediately before/after block-level elements
  ...
  // Remove <br> between a block-level closing tag and a paragraph opening tag
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: keep incrementing when single-item <ol> elements are separated by paragraph content
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert object dictionary to array; prefer reddit platform reply over twitter
...
/* Fix ordered list numbering - use CSS counters to keep multiple ol elements numbered consecutively */
.message-text {
