...
// Read maxRounds from query params at initialization so child components get the value immediately
...
const handleGoBack = async () => {
  // Close any running simulation before navigating back to Step 2
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running — check whether the process still needs to be stopped
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh skips the full-screen loading indicator to avoid flicker.
...
onMounted(() => {
  ...
  // Log maxRounds config (value already read from query params at initialization)
  if (maxRounds.value) {
