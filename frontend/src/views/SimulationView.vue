 ...
 * When the user navigates back from Step 3 to Step 2, the simulation is assumed to be exited.
 */
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful close threw an exception, attempt force stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failure to check environment status does not block the subsequent flow
...
onMounted(async () => {
  ...
  // Check and stop any running simulation (user navigated back from Step 3)
